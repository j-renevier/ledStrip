...
const useWebSocket = (url, onMessageCallback = null, autoConnect = false) => {
  ...
  const open = useCallback(() => {
    ...
    try {
      ...
      socket.addEventListener('message', (event) => {
        console.log(event)
        setMessages(prev => [...prev, {value: `Response received`, level: 'success', update: Date.now()}, {value: event.data, update: Date.now()}]);
